...
do($=jQuery)->
  ...
  count = 0
  $.timeliner = (options) ->
    if ($.timeliners == null or $.timeliners == undefined)
      $.timeliners = {
        options: []
      }
  ...
  startTimeliner = (options)->
    ...
    if ($(settings.timelineContainer).data('started'))
      ...
      $(settings.timelineContainer).on "click", settings.timelineSectionMarker, ->

        # number of minor events under this major event
        numEvents = $(this).parents(settings.timelineSection).find(settings.timelineTriggerContainer).length

        # number of minor events already open
        numOpen = $(this).parents(settings.timelineSection).find('.open').length
      ...
      # All Markers/Events
      el = settings.timelineContainer + " " + ".timeline-toggle"
      $(el).click ->
